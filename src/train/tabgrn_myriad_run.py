...
Both presets use full_finetune / full_finetune_dirichlet (1024 genes, 100-cell
context window).  Step count is computed from N_EPOCHS × non-day-11 cell count
so that every training cell is seen as a query at least N_EPOCHS times.
...
Optional env vars:
    ...
    N_EPOCHS       — passes over the non-day-11 query pool (default: 3)
...
def main() -> None:
    ...
    # Epoch-based step count: every non-day-11 cell is a query N_EPOCHS times
    n_train_cells = sum(1 for d in dataset.collection_day if int(d) != 11)
    n_steps = N_EPOCHS * n_train_cells
    ...
    print(f"  n_steps        : {n_steps}  ({N_EPOCHS} × {n_train_cells})")
    print(f"  eval_every     : {EVAL_EVERY}  (checkpoint interval)")
    print(f"  max_genes      : {cfg.data.max_genes}")
    print(f"  context        : {cfg.context.n_bins} bins × {cfg.context.cells_per_bin} cells"
          f"  = {cfg.context.n_bins * cfg.context.cells_per_bin} anchors/step")
